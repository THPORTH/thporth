...
class Scraper:
    def __init__(self, scraper_id, service, n=0, m=0):
        ...
        self.chrome_prefs["profile.default_content_settings"] = {
            "images": 2}
        self.chrome_prefs["profile.managed_default_content_settings"] = {
            "images": 2}
    ...
    def scrape_data(self):
        ...
        for element in elements:
            if elements.index(element) % 1000 == 0:
                print(
                    f'\t-> Approx. {len(elements) - elements.index(element)} elements left')
            ...
            try:
                if element.name == "div" and "date-events__sport-header-date" in element.get("class", [])[0]:
                    ...
                    event_time = element.find(
                        "div", class_="event__info--time").text

                    event_channels_holder = element.find(
                        "ul", class_="event__tags")

                    event_channels = event_channels_holder.find_all(
                        "span", class_="channel-text")
...
print('===== UPLOADING INTO MySQL OPERATION  =====')
...
query = 'Delete from ThporthToday'
...
for sport_event in all_data:
    if counter % 1000 == 0:
        print(f"\n\t\tInserted {counter} rows...")
    query = "Insert into ThporthToday (home_team,away_team,event_name,event_time,channels,sports_event,league_name,event_date,event_counter) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    db_cursor.execute(query, sport_event)
    connection.commit()
...
print('===== UPLOADING INTO GSHEETS OPERATION  =====')
scope = ["https://spreadsheets.google.com/feeds",
         "https://www.googleapis.com/auth/drive"]
creds = ServiceAccountCredentials.from_json_keyfile_name(
    'scraper-396615-5f864a78d08c.json', scope)
...
for worksheet in worksheets:
    try:
        ...
        worksheet.update("A1:G1", [
                         ['Home Team', 'Away team', 'Event', 'Time', 'Channel', 'League', 'Sport']])
...
for sport_event in all_data:
    ...
    try:
        ...
        worksheet = spreadsheet.add_worksheet(
            title=sport_event[-2], rows="1000", cols="20")
    ...
    print(
        f'\t-> Uploading to Sheet "{sport_event[-2]}" | Row "{row_counter}"...')
